...
Package-main-bundle numbers understate the official stack. A production Vite React app that only connects Ethereum and shows an address transfers **10.95 kB** on first load with Konekt, **145.74 kB** with `@walletconnect/ethereum-provider@2.23.10`, **18.36 kB** with Konekt UI, and **721.26 kB** with `@reown/appkit@1.8.23`. After every lazy chunk is counted, those become **33.68 kB**, **538.06 kB**, **44.83 kB**, and **1079.28 kB**. React is marked external in all four builds.

| Vite app | First load | Overall |
| --- | ---: | ---: |
| Konekt | **10.95 kB** | **33.68 kB** |
| `@walletconnect/ethereum-provider@2.23.10` | **145.74 kB** | **538.06 kB** |
| Konekt + `konekt-ui` | **18.36 kB** | **44.83 kB** |
| `@reown/appkit@1.8.23` + ethers adapter | **721.26 kB** | **1079.28 kB** |

Headless Konekt is **92.5%** smaller on first load and **93.7%** smaller overall. With a wallet modal, Konekt is **97.5%** smaller on first load and **95.8%** smaller overall than AppKit. The four apps live in `packages/size-walletconnect`, `packages/size-appkit`, `packages/size-konekt`, and `packages/size-konekt-ui`. They share React 19 and Vite; the AppKit app turns email, socials, swaps, on-ramp, and analytics off.

The library-only path is still **14.75 kB** through the first encrypted message (9.91 kB initially and a 4.84 kB lazy cipher chunk) plus **12.56 kB** for the wallet modal and styles, for a **27.31 kB** connect stack before React. That is the figure `pnpm size` enforces. The Vite table is what a browser actually downloads.
