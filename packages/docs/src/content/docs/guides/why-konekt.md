...
Package-main-bundle numbers understate the official stack. A production Vite React app that only connects Ethereum and shows an address transfers **10.95 kB** on first load with Konekt, **145.74 kB** with `@walletconnect/ethereum-provider@2.23.10`, **18.98 kB** with Konekt UI, and **721.26 kB** with `@reown/appkit@1.8.23`. After every lazy chunk is counted, those become **33.68 kB**, **538.06 kB**, **45.44 kB**, and **1079.28 kB**. React is marked external in all four builds.

| Vite app | First load | Overall |
| --- | ---: | ---: |
| Konekt | **10.95 kB** | **33.68 kB** |
| `@walletconnect/ethereum-provider@2.23.10` | **145.74 kB** | **538.06 kB** |
| Konekt + `konekt-ui` | **18.98 kB** | **45.44 kB** |
...
The library-only path is still **14.75 kB** through the first encrypted message (9.91 kB initially and a 4.84 kB lazy cipher chunk) plus **13.28 kB** for the wallet modal and styles, for a **28.03 kB** connect stack before React. That is the figure `pnpm size` enforces. The Vite table is what a browser actually downloads.
